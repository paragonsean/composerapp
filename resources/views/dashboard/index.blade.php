            ...
            <div class="row mb-3">
                ...
                <div class="col-12 col-lg-auto mt-3 mt-lg-0">
                    <ul class="nav nav-pills small">
                        <li class="nav-item">
                            <a href="{{ route('dashboard', ['overview' => 'month']) }}" class="nav-link py-1 px-2 {{ request()->input('overview') != 'total' ? 'active' : '' }}" href="#">{{ __('This month') }}</a>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('dashboard', ['overview' => 'total']) }}" class="nav-link py-1 px-2 {{ request()->input('overview') == 'total' ? 'active' : '' }}" href="#">{{ __('All time') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
                        ...
                        <div class="card-body d-flex align-items-center">
                            <div class="d-flex flex-column justify-content-center flex-grow-1">
                                ...
                                <div class="font-weight-bold h4 mb-0">
                                    {{ number_format((request()->input('overview') != 'total' ? Auth::user()->documents_month_count : Auth::user()->documents_total_count), 0, __('.'), __(',')) }}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card border-0 h-100 shadow-sm">
                            <div class="card-body d-flex">
                                <div class="d-flex flex-column justify-content-center flex-grow-1">
                                    ...
                                    <div class="font-weight-bold h4 mb-0">
                                        {{ number_format((request()->input('overview') != 'total' ? Auth::user()->words_month_count : Auth::user()->words_total_count), 0, __('.'), __(',')) }}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card border-0 h-100 shadow-sm">
                            <div class="card-body d-flex">
                                <div class="d-flex flex-column justify-content-center flex-grow-1">
                                    ...
                                    <div class="font-weight-bold h4 mb-0">
                                        {{ number_format((request()->input('overview') != 'total' ? Auth::user()->images_month_count : Auth::user()->images_total_count), 0, __('.'), __(',')) }}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-12 col-md-3 p-2">
                    <div class="card border-0 shadow-sm">
                        <div class="card border-0 h-100 shadow-sm">
                            <div class="card-body d-flex align-items-center">
                                <div class="d-flex flex-column justify-content-center flex-grow-1">
                                    <div class="d-flex align-items-center text-muted text-truncate mb-1">
                                        <div>{{ __('Time saved') }}</div> <div class="d-flex align-content-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}" data-tooltip="true" title="{{ __('This number is calculated for an average of :words words per minute.', ['words' => 40]) }}">@include('icons.info', ['class' => 'text-muted width-4 height-4 fill-current'])</div>
                                    </div>

                                    <div class="font-weight-bold h4 mb-0">
                                        @if((request()->input('overview') != 'total' && Auth::user()->documents_month_count == 0) || (request()->input('overview') == 'total' && Auth::user()->documents_total_count == 0))
                                            0
                                        @else
                                            {{ (clone $now)->diffForHumans((clone $now)->subMicroseconds((((request()->input('overview') != 'total' ? Auth::user()->words_month_count : Auth::user()->words_total_count) / 40) * 60 * 1000 * 1000)), ['syntax' => true, 'parts' => 2, 'short' => true]) }}
                                        @endif
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-12 col-md-6 col-lg-4 p-2" data-template-title="{{ __($template->name) }}">
                        <div class="card border-0 h-100 shadow-sm">
                            <div class="card-body d-flex align-items-center text-truncate">
                                ...
                                <a href="{{ route($template->route) }}" class="text-dark font-weight-medium stretched-link text-decoration-none text-truncate mx-3">{{ __($template->name) }}</a>
                                                            ...
                                                            <div class="text-truncate">
                                                                ...
                                                                <div class="d-flex align-items-center">
                                                                    ...
                                                                    <div class="text-muted text-truncate small cursor-help" data-tooltip="true" title="{{ $document->url }}">
                                                                        {{ Str::substr(strip_tags($document->result) ?? __('No data') . '.', 0, 100) }}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
