...
trait ImageTrait
{
     ...
     */
    protected function imagesStore(Request $request, string $prompt = null)
    {
        $response = $this->fetchImages($request, $prompt);
     ...
     * @throws GuzzleException
     */
    private function fetchImages(Request $request, $prompt)
    {
        ...
        $response = $httpClient->request('POST', 'https://api.openai.com/v1/images/generations',
            [
                ...
                'json' => [
                    'prompt' => trim(preg_replace('/(?:\s{2,}+|[^\S ])/', ' ', $request->input('description'))) . ($request->input('style') ? '. ' . __('The image should have :style style.', ['style' => $request->input('style')]) : '') . ($request->input('medium') ? '. ' . __('The image should be on a :medium medium.', ['medium' => $request->input('medium')]) : '') . ($request->input('filter') ? '. ' . __('Apply :filter filter.', ['filter' => $request->input('filter')]) : ''),
