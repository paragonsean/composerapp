...
class DocumentController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $content->insertOne([__('ID'), __('Name'), __('Result'), __('Words'), __('Updated at'), __('Created at')]);
        foreach ($documents as $document) {
            $content->insertOne([$document->id, $document->name, $document->result, $document->words, $document->updated_at->tz($request->user()->timezone ?? config('app.timezone'))->format(__('Y-m-d')), $document->created_at->tz($request->user()->timezone ?? config('app.timezone'))->format(__('Y-m-d'))]);
        }
